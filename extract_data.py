...
def calculate_alt_bounding_box(altitude_data):
    ...
    maximum_altitude = int(float(altitude_data[0]))
    minimum_altitude = int(float(altitude_data[0]))
    for altitude_point in altitude_data:
        if int(float(altitude_point)) > maximum_altitude:
            maximum_altitude = int(float(altitude_point))
        elif int(float(altitude_point)) < minimum_altitude:
            minimum_altitude = int(float(altitude_point))
...
def calculate_lat_lon_bounding_box(latitude_data, longitude_data):
    ...
    #maximum latitude
    lat_ne_bounding_box = int(float(latitude_data[0]))
    #minimum latitude
    lat_sw_bounding_box = int(float(latitude_data[0]))
    for latitude_point in latitude_data:
        if int(float(latitude_point)) > lat_ne_bounding_box:
            lat_ne_bounding_box = int(float(latitude_point))
        elif int(float(latitude_point)) < lat_sw_bounding_box:
            lat_sw_bounding_box = int(float(latitude_point))

    #maximum longitude
    lon_ne_bounding_box = int(float(longitude_data[0]))
    #minimum longitude
    lon_sw_bounding_box = int(float(longitude_data[1]))
    for longitude_point in longitude_data:
        if int(float(longitude_point)) > lon_ne_bounding_box:
            lon_ne_bounding_box = int(float(longitude_point))
        elif int(float(longitude_point)) < lon_sw_bounding_box:
            lon_sw_bounding_box = int(float(longitude_point))
...
def extract_attributes(decoded_data):
    ...
    # will be used for the calculation of POIs
    #distance_between_points = calculate_distance_between_points(latitude_data, longitude_data)
